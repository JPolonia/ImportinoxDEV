!function($) {
    ...
    BootstrapTableFilter.FILTER_SOURCES = {
        range: {
            search: false,
            rows: [
                {id: 'lte', label: 'Menor  que <input class="form-control" type="text">'},
                {id: 'gte', label: 'Maior que<input class="form-control" type="text">'},
                {id: 'eq', label: 'Igual <input class="form-control" type="text">'}
            ],
    ...
    BootstrapTableFilter.prototype.initFilters = function() {
        var that = this;
        this.$buttonList.append('<li class="remove-filters"><a href="javascript:void(0)" ><span class="glyphicon glyphicon-remove" style="color:#D43F3A;margin-top: 5px; margin-right:10px;"></span><label >Remover filtros</label></a></li>');
    ...
    BootstrapTableFilter.prototype.fillFilterOptions = function(field, data, cls) {
        ...
        $.each(data, function(i, row) {
            ...
            /*Filter by stock imgs*/
            labelThis = rowLabel(row);
            if(field=='stock'){
                switch(option) {
                    case 'A':
            ...
            filter.$dropdownList.append($('<li data-val="' + option + '" class="' + cls + '"><a href="javascript:void(0)"><label><input type="checkbox"  class="filter-enabled"' + (checked ? ' checked' : '') + '> ' + labelThis + '</label></a></li>'));
    ...
    BootstrapTableFilter.prototype.addFilter = function(filter) {
        ...
        if(filter.field != "state"){
            ...
            this.$buttonList.append('<li data-filter-field="' + filter.field + '"><a href="javascript:void(0)"><input type="checkbox" class=""> ' + filter.label + '</a></li>');

            this.trigger('add-filter', filter);

            /*Set default columns*/
            if(filter.field == "material" || filter.field == "acabamento" || filter.field == "stock"){
                filter.enabled="true";
            }
    ...
    BootstrapTableFilter.prototype.enableFilter = function(field) {
        ...
        /*Remove search bar for stock*/
        if (fType.search && filter.field != "stock") {
            filter.$dropdownList.append($('<li class="static"><span><input type="text" class="form-control search-values" placeholder="Pesquisar"></span></li>'));
